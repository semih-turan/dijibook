...
const mergeResultsText = results => {
  ...
  if (results && results.length > 0) {
    for (let i = 0; i < results.length; i++) {
      str += results[i].barcodeFormatString + ': ' + results[i].barcodeText + ' \n';
    }
  } else {
...
class BarcodeRead extends React.Component {
  ...
  useImagePicker = imagePickerLauncher => {
    this.reader.updateRuntimeSettings(EnumDBRPresetTemplate.IMAGE_SPEED_FIRST).catch(err => {
      console.log(err);
    });
  ...
  render() {
    ...
    if (results && results.length > 0) {
      for (var i = 0; i < results.length; i++) {
        barcode_text += results[i].barcodeText;
      }
    }
